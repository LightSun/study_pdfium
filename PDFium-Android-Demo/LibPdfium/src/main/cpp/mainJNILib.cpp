...
extern "C" JNI_FUNC(void, PdfiumCore, nInsertImage)(JNI_ARGS, jlong docPtr, jint pageIndex,jobject bitmap,
   ...
   /* auto imgObj = FPDFPageObj_NewImageObj(doc->pdfDocument);
    ...
    FPDFImageObj_SetMatrix(imgObj, a, b, c, d, e, f);
    //FPDFPageObj_Transform(imgObj, a, b, c, d, e, f);
    //FPDFPageObj_Transform(imgObj, 1, 0, 0, 1, 200, 600);

    auto appendImage = FPDFAnnot_AppendObject(anno, imgObj);
    auto annoCount = FPDFPage_GetAnnotCount(page);
    //auto result = FPDFPage_GenerateContent(page);

    FPDFBitmap_Destroy(pdfBitmap);
    LOGI("FPDFAnnot_AppendObject = %d, annoCount = %d", appendImage, annoCount);*/
    ...
    auto image_bitmap = FPDFBitmap_Create(kBitmapSize, kBitmapSize, 1);
    FPDFBitmap_FillRect(image_bitmap, 0, 0, kBitmapSize, kBitmapSize,
                        0xeeeecccc);
    kBitmapSize, FPDFBitmap_GetWidth(image_bitmap);
    (kBitmapSize, FPDFBitmap_GetHeight(image_bitmap));
    ...
    (FPDFImageObj_SetMatrix(image_object, kBitmapSize, 0, 0,
                                       kBitmapSize, 0, 0));
...
static void readAllImpl(JNIEnv *env, jlong doc_ptr, jint page_index, jmethodID mid,jobject cb){
   ...
   // int flags = FPDF_ANNOT_FLAG_INVISIBLE | FPDF_ANNOT_FLAG_HIDDEN | FPDF_ANNOT_FLAG_PRINT ;
    auto c = FPDFPage_GetAnnotCount(page);
    LOGD("FPDFPage_GetAnnotCount: %d", c);
    for (int i = 0; i < c; ++i) {
        auto anno = FPDFPage_GetAnnot(page, i);
        int flags = FPDFAnnot_GetFlags(anno);
        LOGD("flags: %d", flags);
       // int flags = FPDFAnnot_GetAP(anno);

        //need SetAP first. or else cause crash
        FPDFAnnot_SetAP(anno, FPDF_ANNOT_APPEARANCEMODE_NORMAL, 0);
        //FPDFAnnot_SetFlags(anno, flags);
        // auto subtype = FPDFAnnot_GetSubtype(annot);
        auto oc = FPDFAnnot_GetObjectCount(anno);
        LOGD("FPDFAnnot_GetObjectCount: %d", oc);
        if(oc > 0){
